...
class pt_thumbnail {
	...
	function pt_thumbnail ($settings, $img_url='', $arg='', $default = false) {
		...
		$this->the_image	= $this->sanitize_url($img_url);
	...
	/****************************************************************/
		...
		if (isset($new_args['DIRNAME'])) $this->dirname = true;
	...
	function get_thumbnail() {
		...
		// First, replace domain by server path if image is on server
		...
		if ($this->dirname) {
			$url_dec = explode('/', $dest_path['dirname']);
			$dirname = sanitize_file_name(end($url_dec));
		} else $dirname='';
		...
		// if file has to be generated, generates thumbnails
			...
			$thumb->setImageType($this->extension);
	                ...
	                // Resize image
			$thumb->resize($this->resize_width, $this->resize_height, $this->crop_x, $this->crop_y, $this->keep_ratio);
	...
	function nameFilter($rename_to) {
		...
		// Filters gallery2 names
		if (stripos($rename_to, "g2_itemId=")) {
...
class pt_post_thumbnail {
	...
	function GetImgHTML() {
		...
		if ($this->LB_effect && $this->use_hs) {
			...
			if ($this->showpost || $this->showlink || (!$this->has_image && !$this->has_media && !$this->has_youtube)) {
				...
				$h->set_borders($this->settings['hsframe'], $this->settings['ovtopframe']);
			...
			// Otherwise, links to picture
				...
				$h->set_href_text('', $this->myclassimg.$this->align);
				$h->set_myclasshref($this->myclasshref);
				if ($this->has_caption)
		...
		// Starts output using no javascript library
			...
			if ($this->showpost || !$this->has_image)
				$this->html = "\n".'<a href="'.$this->post_url.'"';
			elseif ($this->showlink)
				$this->html = "\n".'<a href="'.$this->default_link.'"';
	                else
				$this->html = "\n".'<a href="'.$this->the_image.'"';

	                $this->html .= ' title="'.htmlspecialchars($this->title).'"';
			if ($this->myclasshref !='') $this->html .= ' class="'.$this->myclasshref.'"';
			$this->html .= '>';

			$this->html .= "\n\t".'<img src="'.$this->thumb_url.'" alt="'.htmlspecialchars($this->alt_text).'"';
	...
	function check_stream() {
		...
		$the_image = '';
               // Tag for Youtube
		if (
			(stripos($this->post->post_content, 'http://youtube.com/watch') !== false) ||
			(stripos($this->post->post_content, 'http://www.youtube.com/v/') !== false) ||
			(stripos($this->post->post_content, '[youtube') !== false)) 
		{
			$the_image = POSTHUMB_ABSPATH.'images/youtube.png';
		}


                // Tag for Dailymotion
		elseif (
			(stripos($this->post->post_content, 'http://www.dailymotion.com/swf') !== false) || 
			(stripos($this->post->post_content, 'http://www.dailymotion.com/video') !== false) ||
			(stripos($this->post->post_content, '[dailymotion') !== false))
		{
			$the_image = POSTHUMB_ABSPATH.'/images/dailymotion.png';
		}

		// Tag for googlevideo
		elseif (
			(stripos($this->post->post_content, 'http://video.google.fr/videoplay') !== false) || 
			(stripos($this->post->post_content, 'http://video.google.com/videoplay') !== false) || 
			(stripos($this->post->post_content, '[googlevideo') !== false)) 
		
		{
			$the_image = POSTHUMB_ABSPATH.'/images/gvideo.png';
		} 
	...
	function Return_Title ($showtitle='') {
		...
		$ret = "\n".'<ul style = "width:'.$this->resize_width.'">';
		$sep = ', ';
		while ($arg <>'') :
	...
	function GetVideo ($name, $file, $image, $play_width, $play_height) {
		...
		if (class_exists('pt_highslide')) {
	                ...
	                $replace = SetWordTubeMedia ($file, $image, $play_width, $play_height, $ID, $extension, get_wt_playertype(), get_wt_options());
		...
		elseif (class_exists('pt_thickbox')) {
       	                ...
       	                $replace = SetWordTubeMedia ($file, $image, $play_width, $play_height, $ID, $extension, get_wt_playertype(), get_wt_options(), true);
